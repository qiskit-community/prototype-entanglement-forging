...
from entanglement_forging.core.wrappers.entanglement_forged_vqe import EntanglementForgedVQE
from entanglement_forging.core.wrappers.entanglement_forged_vqe_result import \
    EntanglementForgedVQEResult, OptimalParams
...
class EntanglementForgedGroundStateSolver(GroundStateSolver):
    ...
    def __init__(self, qubit_converter, ansatz, bitstrings, config, orbitals_to_reduce=None):
    ...
    def solve(self, problem, **kwargs) -> EntanglementForgedVQEResult:  # pylint: disable=unused-argument,arguments-differ
        ...
        if not isinstance(problem, ElectronicStructureProblem):
            raise ValueError('This version only supports an ElectronicStructureProblem.')
        ...
        forged_operator = ForgedOperator(qmolecule, self.orbitals_to_reduce)
        classical_energies = ClassicalEnergies(qmolecule, self.orbitals_to_reduce)
        ...
        res = EntanglementForgedVQEResult(
            ...
            auxiliary_results=solver.aux_results + [
                ("optimal_params", OptimalParams(energy=solver._ret.eigenvalue +
    ...
    def evaluate_operators(self,
                           state: Union[str, dict, Result,
                                        ...
                                        OperatorBase],
                           operators: Union[PauliSumOp,
                                            OperatorBase,
                                            list, dict]) -> Union[float, List[float],
                                                                  Dict[str, List[float]]]:
        """Evaluates additional operators at the given state."""
        warnings.warn('evaluate_operators not implemented for '
