...
def make_stateprep_circuits(bitstrings, no_bs0_circuits=True):
    ...
    tensor_prep_circuits = [prepare_bitstring(bs, name='bs' + str(bs_idx))
    ...
    for bs1_idx, bs1 in enumerate(bitstrings):
        for bs2_relative_idx, bs2 in enumerate(bitstrings[bs1_idx + 1:]):
            ...
            qcirc.h(i)
            psi_xplus, psi_xmin = [qcirc.copy(name='bs{}bs{}'.format(bs1_idx,
                                                                     bs1_idx + 1 + bs2_relative_idx)
                                                   + name)
...
def prepare_circuits_to_execute(params, stateprep_circuits,
                                op_for_generating_circuits, var_form, statevector_mode):
    ...
    if type(var_form) != 'entanglement_forging.core.variational_forms.VarFormHopgateForged':  # pylint: disable=unidiomatic-typecheck
    ...
    for prep_circ in [qc.copy() for qc in stateprep_circuits]:
        ...
        if statevector_mode:
            circuits_to_execute.append(wavefn_circuit.copy(name=circuit_name_prefix + 'psi'))
        else:
            circuits_this_stateprep = op_for_generating_circuits.construct_evaluation_circuit(
...
def eval_forged_op_with_result(
        result, w_ij_tensor_states, w_ab_superpos_states, params,
        bitstrings_s, op_for_generating_tensor_circuits,
        op_for_generating_superpos_circuits, richardson_stretch_factors,
        statevector_mode, hf_value,
        add_this_to_mean_values_displayed,
        no_bs0_circuits=True, verbose=False):
    ...
    """
    tensor_state_prefixes = ['bs{}'.format(idx) for idx in range(len(bitstrings_s))]
    pauli_vals_tensor_states_raw = _get_pauli_expectations_from_result(
        result, params, tensor_state_prefixes,
    ...
    pauli_vals_tensor_states_extrap = richardson_extrapolate(pauli_vals_tensor_states_raw,
    ...
    for x in range(num_bitstrings):
        for y in range(num_bitstrings):
            if x == y:
                continue
            bs_string = 'bs{}bs{}'.format(min(x, y), max(x, y))
            superpos_state_prefixes += [bs_string + lin_combo for lin_combo in lin_combos]
            superpos_state_indices += [(x, y)]
    raw_states = _get_pauli_expectations_from_result(result, params, superpos_state_prefixes,
                                                     op_for_generating_superpos_circuits,
                                                     richardson_stretch_factors=
                                                     richardson_stretch_factors,
                                                     statevector_mode=statevector_mode,
                                                     no_bs0_circuits=no_bs0_circuits)
    pauli_vals_superpos_states_extrap = richardson_extrapolate(raw_states,
                                                               richardson_stretch_factors, axis=2)
    ...
    for (pauli_vals_tensor_states, pauli_vals_superpos_states) in [
        ...
        if verbose:
            Log.log('Operator as Schmidt matrix: (diagonals have been shifted by given offset',
                    add_this_to_mean_values_displayed, ')')
    ...
    forged_op_results_extrap, forged_op_results_raw = forged_op_results_w_and_wo_extrapolation  # pylint: disable=unbalanced-tuple-unpacking
    return forged_op_results_extrap, forged_op_results_raw


def _get_pauli_expectations_from_result(result, params, stateprep_strings,
                                        op_for_generating_circuits, statevector_mode,
                                        richardson_stretch_factors=None, no_bs0_circuits=True):
    ...
    if statevector_mode:
        op_matrices = np.asarray([op.to_matrix()
                                  for op in [p[1] for p in op_for_generating_circuits.paulis]])
    pauli_vals = np.zeros((len(stateprep_strings),
                           len(op_for_generating_circuits._paulis),  # pylint: disable=protected-access
                           len(richardson_stretch_factors), 2))
    ...
    for prep_idx, prep_string in enumerate(stateprep_strings):
        if no_bs0_circuits and prep_string == 'bs0':
        ...
        for rich_idx, stretch_factor in enumerate(richardson_stretch_factors):
            circuit_name_prefix = ''.join([circuit_prefix_prefix,
                                           '_richardson{:.2f}'.format(stretch_factor), '_'])
            if statevector_mode:
                psi = result.get_statevector(circuit_name_prefix + 'psi')
                pauli_vals_temp = np.real(np.einsum('i,Mij,j->M', np.conj(psi), op_matrices, psi))
            else:
                ...
                pauli_vals_temp, _ = _eval_each_pauli_with_result(
                    ...
                    result=result, statevector_mode=statevector_mode,
                    use_simulator_snapshot_mode=False,
                    circuit_name_prefix=circuit_name_prefix)

            pauli_vals_alphabetical = [x[1] for x in sorted(list(zip(pauli_names_temp,
                                                                     pauli_vals_temp)))]
            if not np.all(np.isreal(pauli_vals_alphabetical)):
                warnings.warn('Computed Pauli expectation value has nonzero '
                              'imaginary part which will be discarded.')
            pauli_vals[prep_idx, :, rich_idx, 0] = np.real(pauli_vals_alphabetical)
    return pauli_vals


def _eval_each_pauli_with_result(tpbgwpo, result, statevector_mode,
                                 use_simulator_snapshot_mode=False,
                                 circuit_name_prefix=''):
    ...
    for basis, p_indices in tpbgwpo._basis:  # pylint: disable=protected-access
        ...
        paulis = [tpbgwpo._paulis[idx] for idx in p_indices]  # pylint: disable=protected-access
        paulis = [p[1] for p in paulis]  ## DISCARDING THE WEIGHTS
        means_this_basis, cov_this_basis = compute_pauli_means_and_cov_for_one_basis(paulis, counts)
...
def compute_h_schmidt(pauli_vals_tensor_states, pauli_vals_superpos_states,
    ...
    """
    h_schmidt_diagonal = np.einsum('ij,xi,xj->x', w_ij_tensor_states,
    ...
    h_schmidt_off_diagonals = (1 / 4) * np.einsum('ab,xa,xb->x', w_ab_superpos_states, p_delta_x,
                                                  p_delta_x)
...
def get_optimizer_instance(config):
    ...
    if 'learning_rate' in optimizer_arg_names:
        optimizer_config['learning_rate'] = lambda: powerseries(config.spsa_c0, 0.602, 0)
